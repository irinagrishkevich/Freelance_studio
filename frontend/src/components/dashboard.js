...
export class Dashboard {
    ...
    loadOrdersInfo(orders){
        ...
        document.getElementById('done-orders').innerText = orders.filter(item => item.status === config.orderStatuses.success).length
        document.getElementById('in-progress-orders').innerText = orders.filter(item => [config.orderStatuses.new, config.orderStatuses.confirmed].includes(item.status)).length
        document.getElementById('canceled-orders').innerText = orders.filter(item => item.status === config.orderStatuses.canceled).length
    }
    ...
    loadCalendarInfo(orders){
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            if (orders[i].deadlineDate) {
                const deadlineDate = new Date(orders[i].scheduledDate)
                preparedEvents.push({
            ...
            if (orders[i].completedDate) {
                const completedDate = new Date(orders[i].scheduledDate)
                preparedEvents.push({
                    title: 'Заказ '  + orders[i].number + ' выполнен' + orders[i].freelancer.name,
                    start: completedDate,
